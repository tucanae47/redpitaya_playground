    ...
    int load_bitstream(const char* name) {
        ...
        if (xdevcfg != nullptr) {
            ...
            if (fwrite(bitstream_data.data(), bitstream_data.size(), 1, xdevcfg) != 1) {
                printf("FpgaManager: Failed to write bitstream to xdevcfg\n");
                fclose(xdevcfg);
                return -1;
            }
            ...
            printf("FpgaManager: Cannot open xdevcfg\n");
            return -1;
        }
    }
  ...
  private:
    ...
    int read_bitstream_data(const char* name, std::vector<char>& bitstream_data) {
        ...
        if (fread(bitstream_data.data(), bitstream_data.size(), 1, fbitstream) != 1) {
            printf("FpgaManager: Cannot read bitstream data\n");
            fclose(fbitstream);
            return -1;
        }
    ...
    int check_bitstream_loaded() {
        ...
        if (fprog_done == nullptr) {
            printf("FpgaManager: Failed to open prog_done\n");
            return -1;
        }
        ...
        if (read(fileno(fprog_done), buff.data(), 1) == 1) {
            if (buff[0] == '1') {
                printf("FpgaManager: Bitstream successfully loaded\n");
                fclose(fprog_done);
                return 0;
            } else {
                printf("FpgaManager: Failed to load bitstream\n");
                fclose(fprog_done);
                return -1;
            }
        } else {
            printf("FpgaManager: Failed to read prog_done\n");
            fclose(fprog_done);
            return -1;
        }
    }
